## XBRT ----
plan_xgbTree = drake_plan(
  # Dynamic names
  # ***************************************************
  model.lst = readRDS(file_in(!!xgbTree_model_path)),
  summ_table = write.csv(
    perf.summ.print,
    file = file_out(!!xgbTree_csv_path) ),
  perf_table = write.csv(
    perf.dt,
    file = file_out(!!xgbTree_p_csv_path) ),
  report = rmarkdown::render(
    knitr_in(!!rmd_path),
    output_file = file_out(!!xgbTree_html_path),
    quiet = TRUE ),
  pty.model.name = "XBRT",
  # *************************************************
