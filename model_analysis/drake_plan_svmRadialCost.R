## SVRC ----
plan_svrc = drake_plan(
  # Dynamic names
  # ***************************************************
  model.lst = readRDS(file_in(!!svmRadialCost_model_path)),
  summ_table = write.csv(
    perf.summ.print,
    file = file_out(!!svmRadialCost_csv_path) ),
  perf_table = write.csv(
    perf.dt,
    file = file_out(!!svmRadialCost_p_csv_path) ),
  report = rmarkdown::render(
    knitr_in(!!rmd_path),
    output_file = file_out(!!svmRadialCost_html_path),
    quiet = TRUE ),
  pty.model.name = "SVR",
  # *************************************************
